...
class Live_Weather_Station_Admin {
    ...
    use Options_manipulation, Schedules_Manipulation, Javascript_Array, Datas_Output {
    ...
    /**
     * Merge data from Netatmo and OWM collect.
     *
     ...
     * @access   protected
     */
    protected function merge_data($netatmo, $owm, $comp, $eph) {
        $result = array();
    ...
    public function display_configuration_page() {
        ...
        if ( isset( $_POST['action'] ) && $_POST['action'] == 'do-add-edit-owm' ) {
            $station = $this->construct_station();
     ...
     * @since    2.0.0
     */
    public function construct_station() {
        $result = array();
        $error = 0;
        if ( array_key_exists('station_id', $_POST) &&
            ...
            $result['station_name'] = $_POST['station_name'];
            $result['loc_city'] = $_POST['loc_city'];
            $result['loc_timezone'] = $_POST['loc_tz'];
            $result['loc_country_code'] = $_POST['loc_country_code'];
            $result['loc_altitude'] = (int)$_POST['loc_altitude'];
            ...
            if ($result['loc_latitude'] == '' && $result['loc_longitude'] == '') {
                $coord = Owm_Collector::get_coordinates_via_owm($result['loc_city'], $result['loc_country_code']);
                if (count($coord) > 0) {
